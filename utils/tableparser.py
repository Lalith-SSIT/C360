...
def opportunities(excel_path):
    sales_dictionary = {}
    # accounts_df = pd.read_excel(excel_path, sheet_name="Account")
    opportunities_df = pd.read_excel(excel_path, sheet_name="Opportunity")
    ...
    for _, opp_row in opportunities_df.iterrows():
        ...
        opp_data = opp_row.drop("Opportunity_ID").to_dict()
        for k, v in opp_data.items():
            if hasattr(v, 'isoformat'):
                opp_data[k] = str(v)
        sales_dictionary[opp_id] = opp_data
...
if __name__ == "__main__":
    ...
    sales_dict = opportunities(excel_file)
    with open("opportunities_dict.txt", "w", encoding="utf-8") as f:
        f.write(json.dumps(sales_dict, indent=4, ensure_ascii=False))
